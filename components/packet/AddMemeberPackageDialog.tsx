...
export interface IAddMemberPackageDialogProps {
    open: boolean;
    setOpen?: any;
    idPackage?: any;
    projectId?: any;
...
export default function AddMemberPackageDialog(props: IAddMemberPackageDialogProps) {
    const { open, setOpen, idPackage, projectId } = props;
    const [listUser, setListUser] = React.useState<any>([]);
    const [listUserWork, setListUserWork] = React.useState<any>([]);
    const dispath = useAppDispatch();

    React.useEffect(() => {
        if (!open || !idPackage || !projectId) return;
        dispath(setLoading(true));

        (async () => {
            Promise.all([
                packageApi.getUserPacket(idPackage),
                projectApi.getUserProject(projectId),
            ]).then((res) => {
    ...
    const handleAddUser = async (index: number) => {
        ...
        try {
            ...
            const { data } = await packageApi.addUser(payload);
    ...
    const handleDeleteUser = async (index: number) => {
        try {
            ...
            const { data } = await packageApi.deleteUser(user.idUserManage);
    ...
    const handleAddAdmin = async (index: number) => {
        try {
            ...
            const { data } = await packageApi.updateAdmin(payload);
            setListUserWork((prev: any) => {
